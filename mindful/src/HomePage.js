...
export default class UserHome extends Component {
  ...
  componentDidMount() {
    ...
    var arrDates = Object.keys(groupedPostsDate).map(date => {
      return new Date(date);
    }); //plain array of dates 

    let sortedDates = arrDates.map(date => {
      let yearNum = date.getFullYear();
      let monthNum = date.getMonth();
      return { year: yearNum, month: monthNum };
    }); //array of objects sorted into the dates' year and month
    this.setState({ date: sortedDates });

    let groupYears = _.groupBy(sortedDates, 'year')

    var allYears = Object.keys(groupYears);
    this.setState({ years: allYears });
    console.log(allYears);

    let arrMonths = arrDates.map(date => {
      let monthNum = date.getMonth();
